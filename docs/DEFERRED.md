...
| DF-v2-19 | Round caps + per-scene budget enforcement | deferred-hardening | `maxTurns`/`maxInteractions` validated in lesson config but enforced NOWHERE; PRD §5.4 budget numbers are paper-only (no token counting, no rate limiter anywhere) — the live risk is NO ceiling, not over-limiting | Phase 4 operational floor: enforce config caps in the interaction path (deny-with-trace; child sees the friend warmly wrap up, never a dead button) + per-scene/per-student cost counters at the gateway (operator-visible; premium posture = counters, not hard limits, per founder decision ⑥; cap-reached UX = warm wrap-up per ⑦ default — confirm at scene-mechanics review) | 2026-06-09 audit finding; supersedes PRD §5.4 MVP-era numbers |
